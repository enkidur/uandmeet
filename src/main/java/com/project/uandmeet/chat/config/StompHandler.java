...
import com.project.uandmeet.jwt.JwtTokenProvider;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) {
            ...
            chatMessageRepository.plusMemberCnt(roomId);
            chatMessageRepository.setMemberEnterInfo(roomId, sessionId);
            ...
            String roomId = chatRoomService.getRoomId((String) Optional.ofNullable(message.getHeaders().get("simpDestination")).orElse("InvalidRoomId"));
            chatMessageRepository.removeMemberEnterInfo(sessionId, roomId);
            chatMessageRepository.minusMemberCnt(roomId);
        }
        return message;
    }
}
